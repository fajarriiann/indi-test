        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('sale.add.item') }}" method="post">
                    @csrf
                    @method('post')
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                                ...
                                @else
                                    @foreach ($saleItem as $val)
                                        <tr class="odd:bg-white even:bg-gray-50 border-b">
                                            ...
                                            <td class="px-6 py-4">{{ $val->item->name }}</td>
                                            <td class="px-6 py-4">{{ $val->item->price }}</td>
                                            <td class="px-6 py-4">{{ $val->qty }}</td>
                                            <td class="px-6 py-4">{{ $val->item->price * $val->qty }}</td>
                                            <td class="px-6 py-4">
                                                <a href="{{ route('sale.delete.item', $val->id) }}" class="px-4 py-2 bg-red-600 rounded-md font-semibold text-xs text-white uppercase hover:bg-red-500">Hapus</a>
                                            </td>
                                        </tr>
                                        @php
                                            $total += $val->item->price * $val->qty;
                                        @endphp
                                    @endforeach
                                    <tr class="odd:bg-white even:bg-gray-50 border-b">
                    ...
                    <form action="{{ route('sale.update', $sale->id) }}" method="post" class="mt-7">
                    @csrf
                    @method('put')
                        ...
                        <div class="mb-6 flex items-center">
                            ...
                            <input type="date" id="tgl" name="tgl" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{ $sale->tgl }}" required>
                        </div>
                        <div class="mb-6 flex items-center">
                            ...
                            <select name="customer" id="customer" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                                @foreach ($customer as $key => $val)
                                    <option value="{{ $key }}" {{ $key == $sale->customer_id ? 'selected' : '' }}>{{ $val }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="flex justify-end items-center space-x-2">
